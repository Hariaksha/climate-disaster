...
genuinely fuzzy even for human coders. \emph{[Audit-sample reliability
statistics: TBD, pending hand-coding.]}
...
the relevant article-level subsample), and are \emph{provisional} pending
the independent post-hoc audit described in Section~\ref{sec:methods} --
see the caveat at the end of Section~\ref{sec:results_robustness}.
...
\paragraph{Audit caveat.} All results in Section~\ref{sec:results} are
estimated from the full-sample LLM coding pass and are provisional pending
the independent post-hoc audit described in Section~\ref{sec:methods}
(reliability statistics: \emph{TBD, pending hand-coding}). The project
owner's working expectation, based on the calibration-sample agreement
already obtained ($\kappa=0.81$ for Relevance, $\kappa=1.00$ for
Attribution), is that the audit-sample statistics will clear the reporting
targets, but this has not yet been confirmed on an independent sample and
should be treated as confirmed only once Section~\ref{sec:methods} is
updated with the actual audit numbers.
